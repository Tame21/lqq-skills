...
"""Base/LoRA compilation and ACL graph isolation for Magistral on v0.23.0."""
    ...
    return bool(
        ...
        or (
            ...
            and (
                ...
                or "MistralForCausalLM" in architectures
            )
        )
    )


def _patch_applies(config) -> bool:
    return bool(config.lora_config is not None and _is_mistral3_family(config))


def _specialize_lora(config) -> bool:
    """Limit the integrated patch to Magistral language-model LoRA graphs."""
    return bool(
        _patch_applies(config)
...
def _validate_config(config):
    if not _patch_applies(config):
        return
    if config.use_v2_model_runner:
        raise ValueError("The Magistral LoRA graph/AOT patch supports model runner v1 only")
    if config.lora_config.max_lora_rank > 64:
        raise ValueError("The Magistral LoRA graph/AOT patch supports max_lora_rank <= 64 only")
    if getattr(config.lora_config, "enable_tower_connector_lora", False):
        raise ValueError("The Magistral graph/AOT patch supports language-model LoRA only")
    speculative_config = config.speculative_config
    if speculative_config is not None and speculative_config.method != "mtp":
        raise ValueError("The Magistral LoRA graph/AOT patch supports MTP speculative decoding only")
    parallel = config.parallel_config
    if parallel.prefill_context_parallel_size > 1 or parallel.decode_context_parallel_size > 1:
        raise ValueError("The Magistral LoRA graph/AOT patch does not support context parallelism")
    if getattr(parallel, "enable_dbo", False) or getattr(parallel, "ubatch_size", 0) > 1:
        raise ValueError("The Magistral LoRA graph/AOT patch does not support microbatching")
    if config.model_config.enable_sleep_mode:
        raise ValueError("The Magistral LoRA graph/AOT patch has not been validated with sleep mode")
...
def weak_ref_workspaces(params):
    ...
    # Iterate physical keys so integer keys are not resolved through the
    # current forward descriptor while all captured graphs are weakened.
...
def _lora_manager_init(
    ...
    enabled = vllm_config is not None and _specialize_lora(vllm_config)
...
def _update_lora_metadata(self, mapping, *args, **kwargs):
    ...
    if getattr(self, "_ascend_graph_specialize_lora", False):
        # The base wrapper updates no_lora only during prefill. Decode routing
        # must also follow alternating base and adapter requests.
...
def _variant_callable(self, variant):
    ...
    raise ValueError(f"Unknown Magistral LoRA AOT variant: {variant}")
...
class _AOTVariantDispatcher:
    ...
    def compile_variant(self, model, variant, args, kwargs):
        ...
        if not hasattr(compiled_callable, "aot_compile"):
            raise RuntimeError(f"AOT compile is unavailable for the Magistral LoRA {variant} callable")
    ...
    def save_dirty(self, model):
        ...
        for variant in _AOT_VARIANTS:
            ...
            try:
                ...
                logger.info(
                    "Saved Magistral LoRA %s AOT artifact to %s",
                    variant,
                    path,
                )
            except Exception as error:
                logger.warning(
                    "Unable to save Magistral LoRA %s AOT artifact to %s: %s",
                    variant,
                    path,
                    error,
                )
...
def _save_aot_compiled_function(self):
    ...
    if not isinstance(dispatcher, _AOTVariantDispatcher):
        raise RuntimeError("Expected the Magistral LoRA AOT variant dispatcher")
...
def _wrapper_init(self, compile_prefix="", is_encoder=False):
    config = get_current_vllm_config()
    enabled = _specialize_lora(config) and backends.model_tag == "backbone" and not is_encoder
    if enabled:
        _validate_config(config)
        if config.compilation_config.mode != CompilationMode.VLLM_COMPILE:
            raise ValueError("Magistral LoRA graph isolation requires CompilationMode.VLLM_COMPILE")
        if config.compilation_config.dynamic_shapes_config.evaluate_guards:
            raise ValueError("Magistral LoRA graph isolation requires dynamic_shapes_config.evaluate_guards=false")
    ...
    self._ascend_base_callable = _compile_base_variant(self, base_prefix, "ascend_base")
    self._ascend_base_one_callable = _compile_base_variant(self, base_one_prefix, "ascend_base_one")
...
def _set_punica_no_lora(model_runner, no_lora):
    seen = set()
...
def _warmup_and_capture(self, desc, cudagraph_runtime_mode, *args, **kwargs):
    context = _force_lora_count(self, desc.num_active_loras) if _specialize_lora(self.vllm_config) else nullcontext()
    with context:
...
def capture_model(self):
    if _specialize_lora(self.vllm_config):
...
def _install():
    ...
    if getattr(
        ...
        raise RuntimeError("Remove the external v0.23 LoRA graph patch before using the integrated patch")
    # Existing set_* functions resolve GraphParams in their module globals,
    # including aliases imported by model_runner_v1 before worker patch init.
    ...
    logger.info_once("Installed Magistral base/LoRA ACL graph and AOT isolation patch")
